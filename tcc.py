...
#CarlaUE4.exe --myautopilot -windowed
"""
...
global x_history, y_history, yaw_history, speed_history, time_history,\
        throttle_history, steer_history,brake_history,\
        psi_history, sigma_history, cross_history,yaw_path_history,\
        new_waypoints_history, P_error_history, I_error_history,D_error_history
...
simulation_time_lock = threading.Lock()
...
def remove_actor(client, actor_filter):
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.my_autopilot_enabled = False
    ...
    def restart(self):
        ...
        while self.player is None:
            if not self.map.get_spawn_points():
                ...
                sys.exit(1)
            try:
                # spawn_point = carla.Transform(carla.Location(x=-52, y=110, z=1),
                #
                #                               carla.Rotation(roll=0, pitch=0, yaw=90))
                spawn_point = carla.Transform(carla.Location(x=118, y=306, z=1),
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                ...
                elif event.key == pygame.K_k:  # <-- We add this condition
                    throttle_history = [0]
                    steer_history = [0]
                    brake_history = [0]
                    psi_history = [0]
                    ...
                    yaw_history = []
                    print("inside")
                    world.my_autopilot_enabled = not world.my_autopilot_enabled
                # elif event.key == K_q:
                #     return True
                elif event.key == K_p:
                    world.my_autopilot_enabled = not world.my_autopilot_enabled
                    if world.my_autopilot_enabled:
                        waypoints = ...  # Retrieve or generate your waypoints here
                        #follow_waypoints(waypoints, world.player, world, sync_mode)
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                # Apply control
                if not self._ackermann_enabled:
                    if not world.my_autopilot_enabled:
...
def carla2pix(x,y):
    world_to_map_translation = np.array([544.0, -10748.0])
    map_offset = np.array([-1643.022, -1643.022])
    cm_per_pixel = 16.43
    world_location = np.array([x*100, y*100])
    relative_location = world_location + world_to_map_translation - map_offset
...
def pix2carla(x,y):
    ...
    map_offset = np.array([-1643.022, -1643.022])
    cm_per_pixel = 16.43
    pixel_coord = np.array([x/100, y/100])
    relative_location = pixel_coord * cm_per_pixel
    world_location = relative_location + map_offset - world_to_map_translation
...
def get_vehicle_location(vehicle):
    """
    Returns the current (x, y) location of the vehicle.
    """
...
def apply_vehicle_control(vehicle, throttle, brake, steer):
    """
...
def get_current_pose(vehicle):
    ...
    # yaw = vehicle.get_transform().rotation.yaw/(2*np.pi)
    yaw = np.radians(vehicle.get_transform().rotation.yaw)

    return (x, y, yaw)

def exec_waypoint_nav_demo(args, vehicle,client,world, clock):
    global x_history, y_history, yaw_history, speed_history, time_history, \
        throttle_history, steer_history, brake_history, \
        psi_history, sigma_history, cross_history, yaw_path_history, \
        new_waypoints_history, P_error_history, I_error_history, D_error_history

    DIST_THRESHOLD_TO_LAST_WAYPOINT = 10  # some distance from last position before #2
    # simulation ends

    # Path interpolation parameters
    INTERP_MAX_POINTS_PLOT = 10  # number of points used for displaying
    # lookahead path
    INTERP_LOOKAHEAD_DISTANCE = 5  # lookahead in meters #5
    INTERP_DISTANCE_RES = 0.01  # distance between interpolated points
    ITER_FOR_SIM_TIMESTEP = 10  # no. iterations to compute approx sim timestep
    WAIT_TIME_BEFORE_START = 0.00  # game seconds (time before controller start)
    TOTAL_RUN_TIME = 2000.00  # game seconds (total runtime before sim end)
    ...
    # waypoints_file = 'my_waypoints.txt'
    waypoints_file = 'a_start_waypoints.txt'
    # waypoints_file = 'rrt_start_waypoints.txt'
    waypoints_np = None
    ...
    # measurement_data, sensor_data = vehicle.read_data()
    # sim_start_stamp = measurement_data.game_timestamp / 1000.0

    # Send a control command to proceed to next iteration.
    # This mainly applies for simulations that are in synchronous mode.
    apply_vehicle_control(vehicle, throttle=0.0, steer=0, brake=1.0)
    ...
    for i in range(num_iterations):
        ...
        # timestamp.elapsed_seconds / 1000, timestamp.frame / 1000)

        # Send a control command to proceed to next iteration
        apply_vehicle_control(vehicle, throttle=0.0, steer=0, brake=1.0)
        # Last stamp
        if i == num_iterations - 1:
            # sim_duration = timestamp.elapsed_seconds / 1000.0
            sim_duration = timestamp/ 1000.0
    ...
    # for frame in range(TOTAL_EPISODE_FRAMES):
        ...
        # current_timestamp = float(timestamp.elapsed_seconds) / 1000.0
        with simulation_time_lock:
            snapshot = carla_world.get_snapshot()
            timestamp = snapshot.timestamp.elapsed_seconds
            frame = snapshot.frame
        current_timestamp = float(timestamp) / 1000.0
        ...
        new_waypoints_history.append(new_waypoints)
        ...
        psi, sigma, cross, yaw_path, P_error, I_error,D_error = controller.get_control()
        ...
        yaw_path_history.append(yaw_path)
        P_error_history.append(P_error)
        I_error_history.append(I_error)
        D_error_history.append(D_error)

        #ani = animation.FuncAnimation(fig, animate(data=current_yaw), interval=1000)
    ...
    # Stop the car
    # apply_vehicle_control(vehicle, throttle=0.0, steer=0.0, brake=1.0)
...
class Controller:
    ...
    def update(self, vehicle_location, vehicle_speed):
        # Error is the distance between the vehicle and the waypoint
        error = ((vehicle_location[0] - self.waypoint[0]) ** 2 + (vehicle_location[1] - self.waypoint[1]) ** 2) ** 0.5

        # Control is proportional to error
        control = 1 * error

        # Clamp control values to be between 0 and 1
        throttle = min(max(control, 0), 1)
        brake = 0.0
        steer = 0.0

        return throttle, brake, steer
def is_close_to(location1, location2, threshold=2.0):
    """
    Checks if location1 is close to location2.
    """
    x1, y1 = location1
    x2, y2 = location2
...
def game_loop(args):
    ...
    #waypoints = [(170, 307), (194, 297),(194, 202)]  # Replace with your actual waypoints
    # waypoints2 = [(170, 307, 0), (194, 297, 0), (194, 202, 0)]  # Replace with your actual waypoints
    waypoints2 = [(170, 307, 2),(180, 307, 1),(184, 306, 1),(189, 304, 1),(192, 301, 1),
                  (194, 297, 1), (194, 287, 2),(194, 277, 3),(194, 170, 3)]  # Replace with your actual waypoints

    try:
        client = carla.Client(args.host, args.port)
        client.set_timeout(2000.0)
        client.load_world('Town02') #town 2 e 5 113,302
        ...
        # for actor in actors:
        #     print(actor.type_id)
        # remove_actor(client, 'static.lamppost')
        ...
        while True:
            # print("autopilot status:                                                     ",world.my_autopilot_enabled)
            ...
            if controller.parse_events(client, world, clock, args.sync):  # add args.sync here

                return

            if world.my_autopilot_enabled:
                # follow_waypoints(waypoints,world.player,world,display, controller, clock, args, client)
                if follow_waypoints_thread is None or (follow_waypoints_thread is not None and not follow_waypoints_thread.is_alive()):
                    stop_thread.clear()
                    #follow_waypoints_thread = threading.Thread(target=follow_waypoints2,args=(waypoints2,world.player,world,display, controller, clock, args, client))
                    follow_waypoints_thread = threading.Thread(target=exec_waypoint_nav_demo,args=(args, world.player,client,world, clock))
                    follow_waypoints_thread.start()
                ...
                # print(np.radians(world.player.get_transform().rotation.yaw))

                if follow_waypoints_thread is not None and follow_waypoints_thread.is_alive():
                    stop_thread.set()
        ...
        #     'yaw_path_history': yaw_path_history,
        #     'P_error_history': P_error_history,
        #     'I_error_history': I_error_history,
        #     'D_error_history': D_error_history,
        #
        # })

        data = list(zip_longest(time_history, x_history, y_history,
                                speed_history,yaw_history,throttle_history,steer_history,
                                brake_history,psi_history,sigma_history,cross_history,
                                yaw_path_history,P_error_history,I_error_history,D_error_history,
                                fillvalue=None))
        df = pd.DataFrame(data, columns=['time_history', 'x_history', 'y_history',
                                'speed_history','yaw_history','throttle_history','steer_history',
                                'brake_history','psi_history','sigma_history','cross_history',
                                'yaw_path_history','P_error_history','I_error_history','D_error_history',
                                ])
        df.to_csv(r'C:\Users\gjuliao\Documents\Python Scripts\Localization\CARLA_0.9.14\WindowsNoEditor\PythonAPI\examples\OUT_DATA_v1.csv',
                  index=False, na_rep='')
        df2 = pd.DataFrame(new_waypoints_history)
        df2.to_csv(
            r'C:\Users\gjuliao\Documents\Python Scripts\Localization\CARLA_0.9.14\WindowsNoEditor\PythonAPI\examples\control_path_v1.csv',
            index=False, na_rep='')
        plt.figure(figsize=(10, 10))
        plt.subplot(241)
        plt.plot(speed_history)
        plt.title('speed_history')
        plt.subplot(242)
        plt.plot(yaw_history)
        plt.title('yaw_history')
        plt.subplot(243)
        plt.plot(x_history)
        plt.title('x_history')
        plt.subplot(244)
        ...
        plt.subplot(245)
        plt.plot(time_history, throttle_history)
        plt.title('throttle_history')
        plt.subplot(246)
        plt.plot(time_history, steer_history)
        plt.title('steer_history')
        plt.subplot(247)
        plt.plot(time_history, brake_history)
        plt.title('brake_history')
        plt.subplot(248)
        ...
        plt.figure(figsize=(10, 10))
        plt.subplot(241)
        plt.plot(time_history, psi_history)
        plt.title('psi_history')
        plt.subplot(242)
        plt.plot(time_history, sigma_history)
        plt.title('sigma_history')
        plt.subplot(243)
        plt.plot(time_history, cross_history)
        plt.title('cross_history')
        plt.subplot(244)
        plt.plot(time_history,yaw_path_history)
        plt.title('yaw_path')
        plt.subplot(245)
        plt.plot(time_history, P_error_history)
        plt.title('P_error_history')
        plt.subplot(246)
        plt.plot(time_history, I_error_history)
        plt.title('I_error_history')
        plt.subplot(247)
        plt.plot(time_history, D_error_history)
        plt.title('D_error_history')
        plt.subplot(248)
        plt.plot(time_history)
        plt.title('time_history')
        plt.show()
...
def main():
    ...
    argparser.add_argument(
        '--myautopilot',
        action='store_true',
        help='enable my custom autopilot')
